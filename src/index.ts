...
const getLastDigit = (num: number): number => parseInt(num.toString(10).slice(-1))
 ...
 * @param magazine
 */
const create = (magazine: MagazineInfo): string => {
  const flag = 491
  const reserved = 0
 ...
 * @param janCode
 */
const getMagazineCode = (janCode:string): string|null => {
