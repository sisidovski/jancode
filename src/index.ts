...
const getCheckDigit = (code:Number) => {
  // 13 digits
  ...
  return 10 - parseInt(total.toString(10).slice(-1));
};
...
const create = (magazineCode: Number, year: Number, month: Number) => {
  const flag = 491
  const reserved = 0
  ...
  const checkDigit = getCheckDigit(parseInt(fragment));

  return parseInt(`${fragment}${checkDigit}`);
};

const extractMagazineCode = (janCode:Number) => {
